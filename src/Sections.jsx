      ...
      {dailyData.map((d, i) => (
        <div key={i} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "8px 0", borderBottom: `1px solid ${C.divider}` }}>
          <div style={{ minWidth: 50 }}>
            <div style={{ fontSize: 12, fontWeight: 600, color: d.gross === busiest.gross ? C.greenText : d.gross === quietest.gross ? C.redText : C.white }}>{d.dayName}</div>
            <div style={{ fontSize: 10, color: C.textMuted }}>{d.date}</div>
          </div>
          <div style={{ fontSize: 12, fontWeight: 700, color: C.white, minWidth: 55, textAlign: "right" }}>{fi(d.gross)}</div>
          <div style={{ fontSize: 11, color: C.textMuted, minWidth: 45, textAlign: "right" }}>{d.qty} items</div>
          {d.transactions && <div style={{ fontSize: 11, color: C.textSecondary, minWidth: 50, textAlign: "right" }}>{d.transactions} trans</div>}
          {d.avgBasket && <div style={{ fontSize: 11, color: C.accentLight, fontWeight: 600, minWidth: 45, textAlign: "right" }}>{f(d.avgBasket)}</div>}
        </div>
      ))}
...
// ─── COMPETITOR PRICING (monthly — estimated) ───────────────────
export function CompetitorPricingSection({ analysis }) {
  ...
  const topItems = useMemo(() => {
    return [...analysis.items].filter(i => i.hasCost && i.qty >= 3 && !["Tobacco","Lottery","Tobacco Sundr"].includes(i.category))
      ...
      .map(i => {
        const up = Math.round((i.gross / i.qty) * 100) / 100;
        const tescoEst = Math.round((up * 0.9) * 100) / 100;
        const asdaEst = Math.round((up * 0.88) * 100) / 100;
        const gap = Math.round((up - Math.min(tescoEst, asdaEst)) * 100) / 100;
        const verdict = gap <= 0 ? "UPSIDE" : gap < 0.20 ? "IN-LINE" : "RISK";
        return { product: i.product, category: i.category, ourPrice: up, tesco: tescoEst, asda: asdaEst, gap, verdict, qty: i.qty };
      });
  }, [analysis.items]);
  ...
  return (
    <SectionCard title="Competitor Pricing — vs Tesco & Asda" icon="🏷️" noPad>
      <div style={{ padding: "10px 16px", fontSize: 12, color: C.textSecondary }}>Top 15 by volume. Supermarket prices estimated (typical 10-12% convenience premium).</div>
      ...
      <TableRow header cells={[{ v: "Product", flex: 2.5 }, { v: "Us" }, { v: "~Tesco" }, { v: "~Asda" }, { v: "Status" }]} />
      {topItems.map((item, i) => (
        <div key={i}>
          ...
          {expanded === i && (
            <div style={{ padding: "10px 16px", background: C.surface, borderBottom: `1px solid ${C.divider}`, fontSize: 12, color: C.textPrimary, lineHeight: 1.6 }}>
              {item.verdict === "UPSIDE" ? `Priced competitively on ${item.qty} units. Don't raise — this drives footfall.`
                : item.verdict === "RISK" ? `£${item.gap.toFixed(2)} above est. supermarket. ${item.qty} units suggests convenience premium accepted. Monitor volume.`
      ...
      <div style={{ padding: "10px 16px" }}>
        <Insight icon="💡" text="For exact prices, check Tesco.com and Asda.com weekly for your top 15 lines." />
      </div>
    </SectionCard>
  );
}
