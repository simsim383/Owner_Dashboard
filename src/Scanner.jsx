...
function SectionCard({ title, icon, children, accent }) {
  return (
    <div style={{ background: C.surface, borderRadius: 16, border: `1px solid ${C.border}`, marginBottom: 16, overflow: "hidden", ...(accent ? { background: accent } : {}) }}>
      {title && (
...
function StatBox({ label, value, sub, color }) {
  return (
    <div style={{ flex: 1, background: C.bg, borderRadius: 10, padding: "10px 12px", border: `1px solid ${C.border}`, textAlign: "center" }}>
...
function Badge({ text, color, bg }) {
  return (
    <span style={{ display: "inline-block", padding: "3px 10px", borderRadius: 99, fontSize: 10, fontWeight: 700, background: bg || C.greenDim, color: color || C.greenText, letterSpacing: 0.5 }}>
      {text}
    </span>
  );
}
  ...
  try {
    ...
    if (data.status === 1 && data.product) {
      ...
      return {
        ...
        countries: p.countries_tags?.includes("en:united-kingdom") ? "UK" : null,
      };
    }
    return { found: false };
  } catch {
...
// ── Main EPOS product lookup ─────────────────────────────────────────
// Matches barcode against allDays items, returns aggregated stats
function lookupInEPOS(barcode, allDays) {
  ...
  const avgMargin   = hasCost && totalGross > 0 && totalProfit != null
    ? (totalProfit / matches.reduce((s, i) => s + (i.net || 0), 0)) * 100 : null;

  // Weekly velocity: total qty / total days * 7
  const totalDays   = allDays.length;
  const weeklyVel   = totalDays > 0 ? Math.round((totalQty / totalDays) * 7 * 10) / 10 : 0;
  const dailyAvg    = totalDays > 0 ? Math.round((totalQty / totalDays) * 10) / 10 : 0;
  ...
  // Daily breakdown (last 14 days for mini chart)
  const dailyBreakdown = sorted.slice(-14).map(day => {
    const item = day.items?.find(i => String(i.barcode).trim() === String(barcode).trim());
    return {
      date: day.dates?.start,
      dayName: day.dates?.start ? new Date(day.dates.start + "T12:00:00").toLocaleDateString("en-GB", { weekday: "short" }) : "?",
      qty: item?.qty || 0,
...
function EPOSResultPanel({ epos, offData }) {
  ...
  const trendColor  = epos.trendPct == null ? C.textMuted
    : epos.trendPct > 0 ? C.greenText : epos.trendPct < 0 ? C.redText : C.textMuted;
  ...
  return (
    <div>
      ...
      <div style={{ marginBottom: 16 }}>
        ...
        {/* Trend pill */}
        {epos.trendPct != null && (
          <div style={{ display: "inline-flex", alignItems: "center", gap: 6, padding: "6px 12px", borderRadius: 99, background: epos.trendPct > 0 ? C.greenDim : C.redDim, border: `1px solid ${epos.trendPct > 0 ? "rgba(34,197,94,0.2)" : "rgba(239,68,68,0.2)"}`, marginBottom: 4 }}>
            <span style={{ fontSize: 13 }}>{epos.trendPct > 0 ? "📈" : "📉"}</span>
      ...
      <div style={{ display: "flex", gap: 8, marginBottom: 12, flexWrap: "wrap" }}>
        <StatBox label="Weekly Avg" value={`${epos.weeklyVel}`} sub="units/week" color={C.white} />
        <StatBox label="Total Sold" value={epos.totalQty} sub={`over ${epos.daysInData} days`} color={C.white} />
      ...
      <div style={{ display: "flex", gap: 8, marginBottom: 16 }}>
        ...
        <StatBox
          ...
          color={marginColor}
        />
        <StatBox label="Best Day" value={epos.bestDay || "—"} sub="most sales" color={C.amberText} />
      </div>
      ...
      {!epos.hasCost && (
        <div style={{ padding: "10px 14px", borderRadius: 10, background: C.redDim, border: "1px solid rgba(239,68,68,0.2)", marginBottom: 12, fontSize: 12, color: C.redText, fontWeight: 600 }}>
          ⚠️ No cost entered in ShopMate — profit & margin not tracked for this product.
        </div>
      )}

      {/* Last 7 days vs prev */}
      {epos.prev7Qty > 0 && (
        <div style={{ padding: "12px 14px", borderRadius: 10, background: C.surface, border: `1px solid ${C.border}`, marginBottom: 12 }}>
          <div style={{ fontSize: 11, color: C.textMuted, marginBottom: 6, textTransform: "uppercase", letterSpacing: 0.6 }}>Last 7 Days vs Previous 7 Days</div>
          <div style={{ display: "flex", gap: 16, alignItems: "center" }}>
            <div>
              <div style={{ fontSize: 20, fontWeight: 800, color: C.white }}>{epos.last7Qty}</div>
              <div style={{ fontSize: 10, color: C.textMuted }}>this week</div>
            </div>
            <div style={{ fontSize: 20, color: C.textMuted }}>→</div>
            <div>
              <div style={{ fontSize: 16, fontWeight: 700, color: C.textSecondary }}>{epos.prev7Qty}</div>
      ...
      {epos.dailyBreakdown?.length > 0 && (
        <div style={{ marginBottom: 12 }}>
          <button
            ...
            style={{ display: "flex", justifyContent: "space-between", alignItems: "center", width: "100%", padding: "12px 14px", borderRadius: showBreakdown ? "10px 10px 0 0" : 10, background: C.surface, border: `1px solid ${C.border}`, cursor: "pointer", color: C.white }}
          >
          ...
          {showBreakdown && (
            <div style={{ padding: "12px 14px", borderRadius: "0 0 10px 10px", background: C.surface, border: `1px solid ${C.border}`, borderTop: "none" }}>
              <MiniBarChart data={epos.dailyBreakdown} />
              <div style={{ marginTop: 12 }}>
                {epos.dailyBreakdown.filter(d => d.qty > 0).reverse().slice(0, 7).map((d, i) => (
                  <div key={i} style={{ display: "flex", justifyContent: "space-between", padding: "5px 0", borderBottom: `1px solid ${C.divider}`, fontSize: 12 }}>
                    <span style={{ color: C.textSecondary }}>{d.date}</span>
                    <span style={{ color: C.textMuted }}>{d.dayName}</span>
      ...
      {offData?.found && (offData.brand || offData.nutriScore || offData.quantity) && (
        <div style={{ padding: "12px 14px", borderRadius: 10, background: C.blueDim, border: "1px solid rgba(59,130,246,0.2)", marginBottom: 12 }}>
...
// ── Not-in-store panel (Open Food Facts only) ────────────────────────
  ...
  return (
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 12 }}>
        ...
        {offData.imageUrl && (
          <img src={offData.imageUrl} alt="" style={{ width: 56, height: 56, objectFit: "contain", borderRadius: 8, background: "#fff", padding: 4 }} />
        )}
      </div>
      ...
      <div style={{ padding: "12px 14px", borderRadius: 10, background: C.amberDim, border: "1px solid rgba(245,158,11,0.2)", marginBottom: 12 }}>
        ...
        <div style={{ fontSize: 12, color: C.textSecondary, lineHeight: 1.5 }}>
          This product isn't in your EPOS data. If you're considering ranging it, you have no sales history to compare against yet.
        </div>
      </div>
      ...
      <div style={{ display: "flex", gap: 8, marginBottom: 12 }}>
        ...
        <StatBox label="Barcode" value={barcode} sub="EAN-13" />
      </div>
      ...
      {offData.ingredients && (
        <div style={{ padding: "10px 14px", borderRadius: 10, background: C.surface, border: `1px solid ${C.border}` }}>
          ...
          <div style={{ fontSize: 11, color: C.textSecondary, lineHeight: 1.5 }}>{offData.ingredients.slice(0, 200)}{offData.ingredients.length > 200 ? "…" : ""}</div>
        </div>
      )}
    </div>
  );
...
function ScannerView({ onResult, onClose }) {
  ...
  useEffect(() => {
    ...
    const config = { fps: 10, qrbox: { width: 260, height: 130 }, aspectRatio: 1.0, formatsToSupport: [0, 4, 5, 6, 7, 8] }; // EAN-13, EAN-8, UPC-A, UPC-E etc
    ...
    qr.start(
      ...
      (decodedText) => {
        if (scanning) {
          setScanning(false);
          qr.stop().then(() => onResult(decodedText)).catch(() => onResult(decodedText));
        }
      },
      () => {} // ignore frame errors
    ).catch(err => {
    ...
    <div style={{ position: "fixed", inset: 0, background: "#000", zIndex: 1000, display: "flex", flexDirection: "column" }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "16px 20px", background: "rgba(0,0,0,0.8)", backdropFilter: "blur(10px)" }}>
        <div>
          ...
          <div style={{ fontSize: 11, color: C.textMuted }}>Point at any product barcode</div>
        </div>
      ...
      <div style={{ flex: 1, position: "relative", display: "flex", alignItems: "center", justifyContent: "center" }}>
        <div id="qr-reader" style={{ width: "100%", maxWidth: 500 }} ref={scannerRef} />

        {/* Overlay corners for visual scan guide */}
        <div style={{ position: "absolute", width: 260, height: 130, pointerEvents: "none" }}>
          {[["0 0","0 0"],["0 100%","0 0"],["100% 0","100% 0"],["100% 100%","100% 100%"]].map(([origin, pos], i) => (
            <div key={i} style={{
              position: "absolute",
              width: 24, height: 24,
              top: i < 2 ? 0 : "auto", bottom: i >= 2 ? 0 : "auto",
              left: i % 2 === 0 ? 0 : "auto", right: i % 2 === 1 ? 0 : "auto",
              borderTop: i < 2 ? `3px solid ${C.green}` : "none",
              borderBottom: i >= 2 ? `3px solid ${C.green}` : "none",
              borderLeft: i % 2 === 0 ? `3px solid ${C.green}` : "none",
              borderRight: i % 2 === 1 ? `3px solid ${C.green}` : "none",
            }} />
          ))}
        </div>

        {error && (
          <div style={{ position: "absolute", bottom: 20, left: 20, right: 20, padding: "12px 16px", borderRadius: 12, background: C.redDim, border: `1px solid rgba(239,68,68,0.3)`, color: C.redText, fontSize: 13, textAlign: "center" }}>
            {error}
          </div>
        )}
      </div>
      ...
      <div style={{ padding: "16px 20px", background: "rgba(0,0,0,0.8)", textAlign: "center" }}>
        <div style={{ fontSize: 12, color: C.textMuted }}>Works with EAN-13, EAN-8, UPC barcodes</div>
      </div>
    </div>
  );
}
  ...
  return (
    <div style={{ display: "flex", gap: 8 }}>
      ...
      <button
        ...
        style={{ padding: "12px 16px", borderRadius: 10, background: val.length >= 8 ? C.green : C.surface, border: "none", color: val.length >= 8 ? C.white : C.textMuted, fontSize: 13, fontWeight: 700, cursor: val.length >= 8 ? "pointer" : "default" }}
      >
        Look up
      </button>
    </div>
...
// MAIN EXPORT
// Props:
//   allDays — array of day objects with items[] from Supabase
// ════════════════════════════════════════════════════════════════════
export default function Scanner({ allDays }) {
  const [view, setView]         = useState("home");   // home | scanning | result | notfound
  const [barcode, setBarcode]   = useState(null);
  const [eposData, setEposData] = useState(null);
  const [offData, setOffData]   = useState(null);
  const [loading, setLoading]   = useState(false);
  const [history, setHistory]   = useState([]);       // recent scans in this session
  ...
  const handleBarcode = useCallback(async (code) => {
    ...
    // 1. Look up in own EPOS data
    const epos = lookupInEPOS(code, allDays);

    // 2. Always fetch Open Food Facts for product metadata
    ...
    if (epos) setView("result");
    else if (off.found) setView("result");
  ...
  if (view === "scanning") return (
    <ScannerView onResult={handleBarcode} onClose={() => setView("home")} />
  );
      ...
      {view === "home" && (
        <SectionCard title="Product Scanner" icon="📷">
          <div style={{ fontSize: 12, color: C.textSecondary, marginBottom: 20, lineHeight: 1.6 }}>
            Scan any product barcode to see your store's sales data, profit, and performance — or get product details for anything you don't yet stock.
          </div>

          {/* Main scan button */}
          <button
            onClick={() => setView("scanning")}
            style={{ width: "100%", padding: "20px 16px", borderRadius: 14, border: `2px solid ${C.green}`, background: C.greenDim, color: C.green, fontSize: 16, fontWeight: 800, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 12, marginBottom: 16, letterSpacing: 0.3 }}
          >
      ...
      {loading && (
        <SectionCard>
          <div style={{ textAlign: "center", padding: "24px 0" }}>
            ...
            <div style={{ fontSize: 14, fontWeight: 700, color: C.white, marginBottom: 4 }}>Looking up {barcode}…</div>
            <div style={{ fontSize: 12, color: C.textMuted }}>Checking your EPOS data & product database</div>
          </div>
        </SectionCard>
      )}
        ...
        <>
          ...
          <SectionCard>
            <div style={{ textAlign: "center", padding: "16px 0 8px" }}>
              ...
              <div style={{ padding: "12px 14px", borderRadius: 10, background: C.surface, border: `1px solid ${C.border}`, textAlign: "left", fontSize: 12, color: C.textMuted, lineHeight: 1.6 }}>
                This could mean the product isn't in your EPOS yet, or it's a non-food item not covered by the product database.
              </div>
            </div>
          </SectionCard>
          <button
